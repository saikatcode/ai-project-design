      ...
      "pastMeetingSummaries": [
        {
          ...
          "summaryText": "string — scope TBD: full summary text, overview paragraph only, or overview + discussion points. See OP-28."
        }
      ]
    }
  ],
...
| OP-28 | **`pastMeetingSummaries.summaryText` scope**: What portion of a prior meeting's FULL_SUMMARY output should be passed as `summaryText` in the meeting prep request? Options: (A) overview paragraph only — lightweight, captures the meeting gist; (B) overview + discussion points — more context, moderate token cost; (C) full summary text — maximum context but token-heavy for customers with many past meetings. Decision should be made jointly with AI team based on what their prep prompt actually needs. | Pending AI team guidance |
